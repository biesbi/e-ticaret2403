...
// ─── PARTS ────────────────────────────────────────────────────
const COLOR_MAP = {
  'kırmızı':'#ef4444','red':'#ef4444','mavi':'#3b82f6','blue':'#3b82f6',
  'yeşil':'#22c55e','green':'#22c55e','sarı':'#facc15','yellow':'#facc15',
...
function renderParts() {
  ...
  if (statsBar) {
    statsBar.innerHTML = `
      <div class="parts-stat-card" style="--_glow:rgba(59,130,246,0.08)">
        <div class="parts-stat-icon psi-total">📦</div>
        <div><div class="parts-stat-value">${allParts.length}</div><div class="parts-stat-label">Toplam Parça Çeşidi</div></div>
      </div>
      <div class="parts-stat-card" style="--_glow:rgba(34,197,94,0.08)">
        <div class="parts-stat-icon psi-stock">✔</div>
        <div><div class="parts-stat-value">${totalStock}</div><div class="parts-stat-label">Toplam Stok Adedi</div></div>
      </div>
      <div class="parts-stat-card" style="--_glow:rgba(245,158,11,0.08)">
        <div class="parts-stat-icon psi-low">⚠</div>
        <div><div class="parts-stat-value">${lowStock}</div><div class="parts-stat-label">Azalan Stok</div></div>
      </div>
      <div class="parts-stat-card" style="--_glow:rgba(248,113,113,0.08)">
        <div class="parts-stat-icon psi-missing">✘</div>
        <div><div class="parts-stat-value">${outOfStock}</div><div class="parts-stat-label">Stokta Yok / Eksik</div></div>
      </div>`;
  }

  let items = allParts.filter(p => {
    const matchSearch = !search || p.name.toLowerCase().includes(search) || p.partNo.toLowerCase().includes(search) || (p.color||'').toLowerCase().includes(search);
